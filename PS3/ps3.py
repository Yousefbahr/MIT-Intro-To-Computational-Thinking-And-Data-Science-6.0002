...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for i in range(int(self.width)):
            for j in range(int(self.height)):
                self.tiles.update({(i, j): self.dirt_amount})
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        actual_tile = math.floor(pos.get_x()), math.floor(pos.get_y())

        self.tiles[actual_tile] -= capacity if capacity <= self.tiles[actual_tile] else self.tiles[actual_tile]
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return len([tile for tile in self.tiles if self.tiles[tile] == 0])
        # raise NotImplementedError
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        return actual_tile in self.furniture_tiles
        #raise NotImplementedError
    ...
    def is_position_valid(self, pos):
        ...
        return actual_tile not in self.furniture_tiles and self.is_position_in_room(pos)
        #raise NotImplementedError
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_pos = Position.get_new_position(self.position, self.direction, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        while True:
            # Reached target number of cleaned tiles
            if min_coverage * room.get_num_tiles() <= room.get_num_cleaned_tiles():
                break
        ...
        means.append(timestep)

    return sum(means) / len(means)
    #raise NotImplementedError


print ('avg time steps: ' + str(run_simulation(num_robots=11, speed=1.0, capacity=1, width=5, height=5,
                                              dirt_amount=3,min_coverage= 1.0, num_trials=10, robot_type=StandardRobot)))
# === Problem 6
#
