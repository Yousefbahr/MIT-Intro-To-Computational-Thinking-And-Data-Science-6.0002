...
def load_map(map_filename):
    ...
    with open(map_filename) as file:
        # Split text bt new lines
        txt = file.read().split("\n")
        for line in txt:
            # Empty line
            if len(line) == 0:
                break
            src, dest, distance, outdoors = line.split()
            a = Node(src)
            b = Node(dest)
            ...
            edge = WeightedEdge(a, b, int(distance), int(outdoors))
            graph.add_edge(edge)
...
# Include the lines used to test.py load_map below, but comment them out
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    # Invalid Nodes
    if not (digraph.has_node(node) and digraph.has_node(end_node)):
    ...
    for i, edge in enumerate(digraph.get_edges_for_node(node)):
        dest = edge.get_destination().get_name()
        # Avoid unnecessary paths
        if path[2] + edge.get_outdoor_distance() > max_dist_outdoors \
                or path[1] + edge.get_total_distance() > best_dist\
                or dest in path[0]:
            continue

        if best_path is None or len(path[0]) < len(best_path):
            # Update path
            path = [path[0] + [dest],
                    path[1] + edge.get_total_distance(),
                    path[2] + edge.get_outdoor_distance()]

            result = get_best_path(digraph, dest, end,
                                   path, max_dist_outdoors, best_dist, best_path)
            # If dead end , update path by removing last node
            if dest != end or result is None:
                path = [path[0][:-1],
                        path[1] - edge.get_total_distance(),
                        path[2] - edge.get_outdoor_distance()]
